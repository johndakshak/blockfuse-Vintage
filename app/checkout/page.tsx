...
export default function CheckoutPage() {
  ...
  // ── Cart state ──────────────────────────────────────────────────────────────
  // Real cart data loaded from GET /cartItems
  const [cartItems, setCartItems]   = useState<CartItem[]>([]);
  const [cartTotal, setCartTotal]   = useState(0);
  const [cartLoading, setCartLoading] = useState(true);
  const [cartError, setCartError]   = useState("");

  // ── Form state (UI-only — not sent to backend) ───────────────────────────────
  // The backend POST /checkout requires NO body.
  // These fields are collected for the user's reference only.
  ...
  async function placeOrder() {
    ...
    // Frontend guard: must have items in cart
  ...
  return (
    <>
      ...
      <div className="pt-[63px] min-h-screen bg-cream">
        ...
        {/* ── Cart load error ── */}
        {!authLoading && !cartLoading && cartError && (
        ...
        {/* ── Empty cart state ── */}
        {!authLoading && !cartLoading && !cartError && cartItems.length === 0 && (
          <div className="max-w-[1060px] mx-auto px-6 py-20 text-center">
            <h2 className="font-cormorant text-[1.8rem] font-semibold text-charcoal mb-3">
              Your cart is empty
            </h2>
            <p className="text-[0.82rem] text-muted mb-6">
              Add some vintage pieces before checking out.
            </p>
            <Link
              href="/products"
        ...
        {/* ── Main checkout form (only shown once cart is loaded and has items) ── */}
        {!authLoading && !cartLoading && !cartError && cartItems.length > 0 && (
                              ...
                              <div>
                                <div className="bg-white border border-charcoal/10 rounded-xl overflow-hidden mb-3">
                                  {[
                                    ...
                                    { k: "Amount",  v: `₦${(cartTotal + shipCost).toLocaleString("en-NG")}` },
                                  ].map(({ k, v }) => (

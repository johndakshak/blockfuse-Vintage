...
// app/components/home/StaffPicks.tsx
//
// Displays a curated selection of products from the backend.
// Uses the same GET /products endpoint as NewThisWeek but shows
// the first 4 products — giving a different slice of the catalogue.
//
// Data flow and Quick Add behaviour are identical to NewThisWeek.
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        // Show the first 4 products — a different selection from NewThisWeek
        // which shows the last 4
        const picks = response.data.slice(0, 4);
        setProducts(picks);
      } catch (err: unknown) {
        setError(
